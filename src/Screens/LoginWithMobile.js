...
const LoginWithMobile = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    if (!mobile) {
      ...
      Toast.show('Send otp in mobile', Toast.LONG);
    ...
    try {
      console.log(mobile)
      setLoading(true);
      const response = await registerUser(mobile, password, confirmPassword);

      if (response.status == true) {
    
        navigation.replace(navigationString.ProfileData, { personal_details: response.personal_details, mobile, email : "" })
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.app_background }}>
      ...
      <CustomHeaderLogin icon_name='left' title="Register with Mobile" onBackPress={handleBackPress} />
    ...
    >
      ...
      <ScrollView 
        ...
        <CustomEditText text_name={'Mobile Number'}
