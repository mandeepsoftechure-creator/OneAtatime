import React, { useContext, useState } from 'react'
import { useTranslation } from 'react-i18next'
import { StyleSheet, Text, View, SafeAreaView, StatusBar, ScrollView, Alert, KeyboardAvoidingView } from 'react-native'
import CustomHeaderLogin from '../components/CustomHeaderLogin';
import { useStatusBar } from '../AppStatusBar/AppStatusBar';
import { BaseColor, useTheme } from '../config/theme';
import CustomEditText from '../components/CustomEditText';
import { responsiveFontSize, responsiveHeight, responsiveScreenWidth, responsiveWidth } from 'react-native-responsive-dimensions';
import RegisterButton from '../components/RegisterButton';
import { Fonts } from '../config/styles/fonts';
import navigationString from '../constant/navigationString';
import Loader from '../components/Loader';
import { forgot_password, registerUser, sendOtp } from '../Api/auth';
import CustomPassword from '../components/CustomPassword';
import Toast from 'react-native-simple-toast';
import withNetworkCheck from '../Utils/withNetworkCheck';

const ForgotPassword = ({ navigation }) => {

  const { t } = useTranslation();
  const { colors } = useTheme();

  const [loading, setLoading] = useState(false);

 
  const [buttonSend, setButtonSend] = useState(false);
  const [buttonOtp, setButtonOtp] = useState(false);

  const [mobile, setMobile] = useState(undefined);
  const [otp, setOtp] = useState(undefined);
  const [verifyOtp, setVerifyOtp] = useState();
  const [password, setPassword] = useState(undefined);
  const [confirmPassword, setConfirmPassword] = useState(undefined);

  const [isMobileEditable, setIsMobileEditable] = useState(true);
  const [isOtpEditable, setIsOtpEditable] = useState(true);
  const [otpVerify, setOtpVerify] = useState(false);


  // useStatusBar('dark-content', colors.background);

  const handleBackPress = () => {
    navigation.goBack();
  };

  const handleSendOtp = async () => {

    if (!mobile || mobile.length < 10) {
      Toast.show('Please enter a valid mobile number', Toast.LONG);
      return;
    }
    try {
      setLoading(true);
      const response = await sendOtp(mobile);

      if (response.status == true) {
        setButtonSend(false)
        setIsMobileEditable(false)
        setVerifyOtp(response.otp)
        console.log('OTP Sent:', response.message);

      } else {
        console.log('OTP Sent:', response.message);
      }
    } catch (error) {

      console.log('Error sending OTP:', error);
    } finally {
      setLoading(false);
    }
  };

  const handleVerifyOtp = async () => {

    if (!otp || otp.length < 4) {
      Toast.show('Please enter a valid otp', Toast.LONG);
      return;
    } else if (verifyOtp != otp) {
      Toast.show('Invalid OTP. Please try again.', Toast.LONG);
      return;
    }

    try {
      console.log(mobile)
      setLoading(true);
      const response = await sendOtp(mobile);

      if (response.status == true) {
        setButtonOtp(false)
        setIsOtpEditable(false)
        setOtpVerify(true)
        console.log('OTP Sent:', response.message);

      } else {
        console.log('OTP Sent:', response.message);
      }
    } catch (error) {

      console.log('Error sending OTP:', error);
    } finally {
      setLoading(false);
    }
  };

  const handleSubmit = async () => {
    if (!mobile) {
      Toast.show('Please enter mobile or email address', Toast.LONG);

    } else if (!verifyOtp) {
      Toast.show('Please Send Otp first', Toast.LONG);

    } else if(!otpVerify) {
      Toast.show('Please verfiy otp', Toast.LONG);
  } else if (!otp) {
    Toast.show('OTP is required', Toast.LONG);
  } else if (!password) {
    Toast.show('Change Password is required', Toast.LONG);
  } else if (password.length < 8) {
    Toast.show('Change Password must be at least 8 characters long', Toast.LONG);
  } else if (!confirmPassword) {
    Toast.show('Confirm Password is required', Toast.LONG);
  } else if (password !== confirmPassword) {
    Toast.show('Passwords do not match', Toast.LONG);
  }else
    try {
      console.log(mobile)
      setLoading(true);
      const response = await forgot_password(mobile, password, confirmPassword);

      if (response.status == true) {
    
        navigation.replace(navigationString.Login)
        Toast.show(response.message, Toast.LONG);

      } else {
        setButtonSend(false);
        setButtonOtp(false);
        setMobile(undefined);
        setOtp(undefined);
        setVerifyOtp(undefined);
        setPassword(undefined);
        setConfirmPassword(undefined);
        setIsMobileEditable(true);
        setIsOtpEditable(true);
        setOtpVerify(false);

        Toast.show(response.message, Toast.LONG);
      }
    } catch (error) {

      console.log('Error response:', error);
    } finally {
      setLoading(false);
    }

  }


  return (
    <View style={{ flex: 1, backgroundColor: colors.app_background }}>

      <Loader visible={loading}></Loader>

      <CustomHeaderLogin icon_name='left' title="Forgot Password" onBackPress={handleBackPress} />

      <KeyboardAvoidingView>
      <ScrollView >

      <CustomEditText text_name={t('email_mobile')}
          style_view={{ backgroundColor: BaseColor.light }}
          placeholder={'Enter...'}
          visibilityButton={buttonSend}
          onTextChange={(text) => { setMobile(text), text.length >= 10 ? setButtonSend(true) : setButtonSend(false) }}
          buttonPress={handleSendOtp}
          isEditable={isMobileEditable}
         // maxLength={10}
          button_title={'Send OTP'}
          //keyboardType={'numeric'}
          text_input={mobile}
          
        />




        <CustomEditText text_name={'OTP'}
          style_view={{ backgroundColor: BaseColor.light }}
          placeholder={'Enter...'}
          visibilityButton={buttonOtp}
          onTextChange={(text) => { setOtp(text), text.length >= 4 ? setButtonOtp(true) : setButtonOtp(false) }}
          buttonPress={handleVerifyOtp}
          isEditable={isOtpEditable}
          maxLength={4}
          button_title={'Verify OTP'}
          keyboardType={'numeric'}
          secureTextEntry={true}
          text_input={otp}

        />

        {otpVerify && (

          <View>
            <CustomPassword text_name={'Change Password'}
              style_view={{ backgroundColor: BaseColor.light }}
              placeholder={'Enter...'}
              onTextChange={(text) => { setPassword(text) }}
            />

            <CustomPassword text_name={'Confirm Password'}
              style_view={{ backgroundColor: BaseColor.light }}
              placeholder={'Enter...'}
              onTextChange={(text) => { setConfirmPassword(text) }}
            />
          </View>
        )}



      </ScrollView>

      </KeyboardAvoidingView>


      <View style={{
        position: 'absolute', bottom: responsiveHeight(0), left: responsiveWidth(0),
        right: responsiveWidth(0), marginBottom: responsiveHeight(2),
        marginHorizontal: responsiveWidth(2)
      }}>

        {/* <Text style={[styles.footerText]}>
          Don't have account? Let's create!
        </Text> */}
        <RegisterButton
          title="Change Password"
          style={{textAlign: 'center', marginHorizontal: responsiveWidth(3)}}
          textStyle={{textAlign: 'center', fontSize: responsiveFontSize(2)}}
          onPress={handleSubmit}
        />

      </View>
    </View>

  )
}

export default withNetworkCheck(ForgotPassword);

const styles = StyleSheet.create({
  footerText: {
    fontSize: responsiveFontSize(1.4),
    color: BaseColor.primary,
    textAlign: 'center',
    marginBottom: responsiveHeight(1.8),
    fontFamily: Fonts.semiBold,

  },
})